function b1()
{
    document.getElementById("c1").style.display="block";
    document.getElementById("h1").style.display="none";
    document.getElementById("hh1").style.display="block";
}

function b2()
{
    document.getElementById("c2").style.display="block";
    document.getElementById("h2").style.display="none";
    document.getElementById("hh2").style.display="block";
}

function b3()
{
    document.getElementById("c3").style.display="block";
    document.getElementById("h3").style.display="none";
    document.getElementById("hh3").style.display="block";
}

function b4()
{
    document.getElementById("c4").style.display="block";
    document.getElementById("h4").style.display="none";
    document.getElementById("hh4").style.display="block";
}

function b5()
{
    document.getElementById("c5").style.display="block";
    document.getElementById("h5").style.display="none";
    document.getElementById("hh5").style.display="block";
}

function b6()
{
    document.getElementById("c6").style.display="block";
    document.getElementById("h6").style.display="none";
    document.getElementById("hh6").style.display="block";
}

function b7()
{
    document.getElementById("c7").style.display="block";
    document.getElementById("h7").style.display="none";
    document.getElementById("hh7").style.display="block";
}

function b8()
{
    document.getElementById("c8").style.display="block";
    document.getElementById("h8").style.display="none";
    document.getElementById("hh8").style.display="block";
}

function b9()
{
    document.getElementById("c9").style.display="block";
    document.getElementById("h9").style.display="none";
    document.getElementById("hh9").style.display="block";
}








function bc1()
{
    document.getElementById("c1").style.display="none";
    document.getElementById("h1").style.display="block";
    document.getElementById("hh1").style.display="none";
}

function bc2()
{
    document.getElementById("c2").style.display="none";
    document.getElementById("h2").style.display="block";
    document.getElementById("hh2").style.display="none";
}

function bc3()
{
    document.getElementById("c3").style.display="none";
    document.getElementById("h3").style.display="block";
    document.getElementById("hh3").style.display="none";
}

function bc4()
{
    document.getElementById("c4").style.display="none";
    document.getElementById("h4").style.display="block";
    document.getElementById("hh4").style.display="none";
}

function bc5()
{
    document.getElementById("c5").style.display="none";
    document.getElementById("h5").style.display="block";
    document.getElementById("hh5").style.display="none";
}

function bc6()
{
    document.getElementById("c6").style.display="none";
    document.getElementById("h6").style.display="block";
    document.getElementById("hh6").style.display="none";
}

function bc7()
{
    document.getElementById("c7").style.display="none";
    document.getElementById("h7").style.display="block";
    document.getElementById("hh7").style.display="none";
}

function bc8()
{
    document.getElementById("c8").style.display="none";
    document.getElementById("h8").style.display="block";
    document.getElementById("hh8").style.display="none";
}

function bc9()
{
    document.getElementById("c9").style.display="none";
    document.getElementById("h9").style.display="block";
    document.getElementById("hh9").style.display="none";
}









function sho1()
{
    document.getElementById("about1").style.display="block";
    document.getElementById("more1").style.display="none";
    document.getElementById("cls1").style.display="block";
}

function sho2()
{
    document.getElementById("about2").style.display="block";
    document.getElementById("more2").style.display="none";
    document.getElementById("cls2").style.display="block";
}

function sho3()
{
    document.getElementById("about3").style.display="block";
    document.getElementById("more3").style.display="none";
    document.getElementById("cls3").style.display="block";
}

function sho4()
{
    document.getElementById("about4").style.display="block";
    document.getElementById("more4").style.display="none";
    document.getElementById("cls4").style.display="block";
}

function sho5()
{
    document.getElementById("about5").style.display="block";
    document.getElementById("more5").style.display="none";
    document.getElementById("cls5").style.display="block";
}

function sho6()
{
    document.getElementById("about6").style.display="block";
    document.getElementById("more6").style.display="none";
    document.getElementById("cls6").style.display="block";
}

function sho7()
{
    document.getElementById("about7").style.display="block";
    document.getElementById("more7").style.display="none";
    document.getElementById("cls7").style.display="block";
}

function sho8()
{
    document.getElementById("about8").style.display="block";
    document.getElementById("more8").style.display="none";
    document.getElementById("cls8").style.display="block";
}

function sho9()
{
    document.getElementById("about9").style.display="block";
    document.getElementById("more9").style.display="none";
    document.getElementById("cls9").style.display="block";
}








function clos1()
{
    document.getElementById("cls1").style.display="none";
    document.getElementById("about1").style.display="none";
    document.getElementById("more1").style.display="block";
}

function clos2()
{
    document.getElementById("cls2").style.display="none";
    document.getElementById("about2").style.display="none";
    document.getElementById("more2").style.display="block";
}

function clos3()
{
    document.getElementById("cls3").style.display="none";
    document.getElementById("about3").style.display="none";
    document.getElementById("more3").style.display="block";
}

function clos4()
{
    document.getElementById("cls4").style.display="none";
    document.getElementById("about4").style.display="none";
    document.getElementById("more4").style.display="block";
}

function clos5()
{
    document.getElementById("cls5").style.display="none";
    document.getElementById("about5").style.display="none";
    document.getElementById("more5").style.display="block";
}

function clos6()
{
    document.getElementById("cls6").style.display="none";
    document.getElementById("about6").style.display="none";
    document.getElementById("more6").style.display="block";
}

function clos7()
{
    document.getElementById("cls7").style.display="none";
    document.getElementById("about7").style.display="none";
    document.getElementById("more7").style.display="block";
}

function clos8()
{
    document.getElementById("cls8").style.display="none";
    document.getElementById("about8").style.display="none";
    document.getElementById("more8").style.display="block";
}

function clos9()
{
    document.getElementById("cls9").style.display="none";
    document.getElementById("about9").style.display="none";
    document.getElementById("more9").style.display="block";
}



